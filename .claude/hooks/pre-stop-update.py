...
output = {
    ...
    "reason": (
        ...
        "documents that should reflect what you accomplished during this session (for example design documents, API specifications, etc.).\n If nothing significant happened, you may skip the update. \n Before invoking the `memory-curator` skill - answer to these 3 questions and invoke the skill only if at least on of the following is true: 1) Is this an important architectural decision or pattern that will be useful for future reference? Will this decision meaningfully affect how we build other features? 2) Is this a problem that is worth remembering and avoiding in the future? 3) If you never will work on this task again, will this insight be useful for future reference?. \n\n"
        "If you were working on a task, update the status, session progress and next steps in "
        f"`{claude_project_dir}/.meridian/tasks/TASK-###/{{TASK-###.yaml,TASK-###-plan.md,TASK-###-context.md}}` with details such as: the current implementation "
